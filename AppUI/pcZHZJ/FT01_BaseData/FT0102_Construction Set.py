...
class PageRelease(unittest.TestCase):
    def setUp(self):
        ...
        # 打开菜单
        WebMenu.full_text(self, "基础数据", "建设单位管理")
    ...
    def test_0101_01_add(self):
        ...
        # 建设单位编码
        dr.find_element_by_id(_baseForm + 'code').send_keys('JSDW-'+random_ssn())
        # 纳税人识别号
        dr.find_element_by_id(_baseForm + 'taxNo').send_keys(random_str(15, 16))
        # 法人
        dr.find_element_by_id(_baseForm + 'legalPerson').send_keys(random_name())
        # 注册地址
        dr.find_element_by_id(_baseForm + 'registeredAddress').send_keys(random_address())
        # 备注
