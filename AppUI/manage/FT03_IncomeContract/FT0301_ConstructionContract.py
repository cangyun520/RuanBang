...
class ConstructionContract(unittest.TestCase):
    def setUp(self):
        ...
        WebMenu.menu(self, '//*[@id="/manage/IncomeContract$Menu"]/li[1]')

    """施工合同-行数据添加"""
    def test_0301_01_add(self):
        """施工合同-行数据添加"""
        dr = self.driver
        cy_check = dr.find_element_by_class_name("ant-breadcrumb-link")

        if "施工合同" in cy_check.text:
            print(cy_check)
        else:
            dr.get_screenshot_as_file(SCREENSHOT_PATH + "Income/test_0301_01_add.png")
            unittest.expectedFailure("test_0301_01_add")
