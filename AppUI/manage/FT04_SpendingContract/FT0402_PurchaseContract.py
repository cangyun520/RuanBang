...
class PurchaseContract(unittest.TestCase):
    def setUp(self):
        ...
        WebMenu.menu(self, '//*[@id="/manage/SpendingContract$Menu"]/li[2]')

    """采购合同-行数据添加"""
    def test_0402_01_add(self):
        """分包合同-行数据添加"""
        dr = self.driver
        cy_check = dr.find_element_by_class_name("ant-breadcrumb-link")

        if "采购合同" in cy_check.text:
            print(cy_check)
        else:
            dr.get_screenshot_as_file(SCREENSHOT_PATH + "Spending/test_0402_01_add.png")
            unittest.expectedFailure("test_0402_01_add")
